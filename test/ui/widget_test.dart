...
class MockNewsViewModel extends Mock implements VideosViewModel {}

class MockUserViewModel extends Mock implements UserViewModel {}

void main() {
  final mockNewsViewModel = MockNewsViewModel();
  when(mockNewsViewModel.fetchTrending).thenAnswer((_) => Future.value());
  when(() => mockNewsViewModel.videos)
  ...
  testWidgets('App widget test', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          videosViewModelProvider.overrideWithValue(mockNewsViewModel),
